/*
 * STMicroelectronics ism303dac_mag_spi.c driver
 *
...
static int ism303dac_mag_spi_read(struct st_common_data *cdata,
...
/* XXX: caller must hold cdata->lock */
static int ism303dac_mag_spi_write(struct st_common_data *cdata,
				  u8 addr, int len, u8 *data)
{
	struct spi_message msg;
	struct spi_device *spi = to_spi_device(cdata->dev);
...
static struct st_sensor_transfer_function ism303dac_mag_spi_tf = {
	.write = ism303dac_mag_spi_write,
	.read = ism303dac_mag_spi_read,
};

#ifdef CONFIG_PM_SLEEP
static int ism303dac_mag_resume(struct device *device)
{
...
static int ism303dac_mag_suspend(struct device *device)
{
	struct spi_device *spi = to_spi_device(device);
	struct st_common_data *cdata = spi_get_drvdata(spi);
...
static SIMPLE_DEV_PM_OPS(ism303dac_mag_pm_ops,
			ism303dac_mag_suspend,
			ism303dac_mag_resume);

#define ISM303DAC_MAG_PM_OPS	(&ism303dac_mag_pm_ops)
#else /* CONFIG_PM_SLEEP */
#define ISM303DAC_MAG_PM_OPS	NULL
#endif /* CONFIG_PM_SLEEP */

#ifdef CONFIG_OF
static const struct of_device_id ism303dac_mag_spi_id_table[] = {
	{ .compatible = "st,ism303dac_mag", },
	{ },
};
MODULE_DEVICE_TABLE(of, ism303dac_mag_spi_id_table);
#endif /* CONFIG_OF */

static int ism303dac_mag_spi_probe(struct spi_device *spi)
{
	int err;
...
MODULE_DEVICE_TABLE(spi, ism303dac_mag_spi_id);

static struct spi_driver ism303dac_mag_spi_driver = {
	.driver = {
		.owner = THIS_MODULE,
		.name = "ism303dac_mag_spi",
		.pm = ISM303DAC_MAG_PM_OPS,
#ifdef CONFIG_OF
		.of_match_table = ism303dac_mag_spi_id_table,
#endif /* CONFIG_OF */
	},
	.probe = ism303dac_mag_spi_probe,
	.remove = ism303dac_mag_spi_remove,
	.id_table = ism303dac_mag_spi_id,
};

module_spi_driver(ism303dac_mag_spi_driver);

MODULE_DESCRIPTION("ism303dac_mag spi driver");

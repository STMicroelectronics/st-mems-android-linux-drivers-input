...
static int lsm303dlhc_acc_resume(struct device *device)
{
...
static int lsm303dlhc_acc_suspend(struct device *device)
{
	struct i2c_client *client = to_i2c_client(device);
	struct lsm303dlhc_acc_status *stat = i2c_get_clientdata(client);
...
static SIMPLE_DEV_PM_OPS(lsm303dlhc_acc_pm_ops,
			lsm303dlhc_acc_suspend,
			lsm303dlhc_acc_resume);

#define LSM303DLHC_ACC_PM_OPS	(&lsm303dlhc_acc_pm_ops)
#else /* CONFIG_PM_SLEEP */
#define LSM303DLHC_ACC_PM_OPS	NULL
...
static void lsm303dlhc_acc_i2c_remove(struct i2c_client *client)
{
	struct lsm303dlhc_acc_status *stat = i2c_get_clientdata(client);
...
#else
static int lsm303dlhc_acc_i2c_remove(struct i2c_client *client)
{
	struct lsm303dlhc_acc_status *stat = i2c_get_clientdata(client);
